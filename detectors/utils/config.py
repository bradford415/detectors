...
def load_config(config_path, cfg=dict()):
    ...
    """
    assert Path(config_path).suffix in [".yml", ".yaml"], "config file must be a yaml file"
...
def merge_dict(dict_one: dict, dict_two: dict, inplace=True) -> dict:
    ...
    def _merge(first_dict, second_dict) -> dict:
        for param in second_dict:
            ...
            if (param in first_dict and isinstance(first_dict[param], dict) and isinstance(second_dict[param], dict)):
