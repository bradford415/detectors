...
def ap_per_class(tp, conf, pred_cls, target_cls):
    ...
    """
    breakpoint() #################START HERE GO THROUGH AND BRIEFLY UNDERSTAND AP CALCULATION ######################
    # Sort by objectness in decreasing order (highest confidence first)
    best_conf_i = np.argsort(-conf)
    tp, conf, pred_cls = tp[best_conf_i], conf[best_conf_i], pred_cls[best_conf_i] # (batch_size*num_preds,)
    ...
    for cls in tqdm.tqdm(unique_classes, desc="Computing AP"):
        ...
        if n_p == 0 and n_gt == 0:
            ...
            fp_count = (1 - tp[cls_i]).cumsum() # select the true positives for the current cls
