...
class PositionEmbeddingSineHW(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        if self.normalize:
            eps = 1e-6
            y_embed = y_embed / (y_embed[:, -1:, :] + eps) * self.scale # column normalize
        ...
        # cosine argument
        dim_tx = torch.arange(self.num_pos_feats, dtype=torch.float32, device=image_patches.device)
        dim_tx = self.temperatureW ** (2 * (dim_tx // 2) / self.num_pos_feats) # denominator
        ...
        # sine argument
        dim_ty = torch.arange(self.num_pos_feats, dtype=torch.float32, device=image_patches.device)
